...
%     savefilename = ['_Pitch_St',num2str(St*100),...
%             '_A_c',num2str(A_c*100),...
%             '_d_c_inf'];
    savefilename = ['_Pitch_St',num2str(St*100),...
        '_A_c',num2str(A_c*100),...
        '_d_c',num2str(d_c*100)];
        
    save(['Data/Parameters',savefilename,'.mat'],'-v7.3')

    fid_Data = fopen(['Data/Data',savefilename,'.txt'],'w');
    fid_PanelProp = fopen(['Data/PanelProp',savefilename,'.txt'],'w');
    fid_WakeProp = fopen(['Data/WakeProp',savefilename,'.txt'],'w');
end
...
for i_t = 1:Ncyc*Nstep+1
    % Start timer.
    tic
    Qinf = sqrt(Q0(1,i_t)^2 + Q0(2,i_t)^2);
...
%         
    ...
    xTE = [xp(end); xp(end) + TEvec(1)*Qinf*delT*TEfac];
    zTE = [zp(end); zp(end) + TEvec(2)*Qinf*delT*TEfac];
...
%         print('-dpng','-r300',['2D_NearBody_',num2str(i_t)]);
    ...
    if free == 1 && i_t > 1
        ...
        a_b = Fx/M;
        Q0(1,i_t) = a_b*delT + Q0(1,i_t-1);
        if i_t == 1
            x_b(1) = a_b*delT^2 + 2*x_b(2);
        else
            x_b(1) = a_b*delT^2 + 2*x_b(2) - x_b(3); 
        end
...
%     sprintf(['Calculating...Time Remaining: ',num2str(EstThour),' hrs ',num2str(EstTmin),' mins ',num2str(EstTsec),' secs']);
    ...
    if SaveData == 1           
        Data = [wakeInd,Q0(1,i_t),Q0(2,i_t),x_b(2),z_b(2),a_b,Fx,Fz,Pow,L,T,Gamma,Cf,Cl,Ct,Cpow,...
            Fx_s,Fx_us,Fz_s,Fz_us,Pow_s,Pow_us,L_s,L_us,T_s,T_us,Cl_s,Cl_us,...
            Ct_s,Ct_us,Cpow_s,Cpow_us];
        fprintf(fid_Data,'%16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f %16.8f\n',Data');
...
Cpow_avg
np = Ct_avg/Cpow_avg
Ec = mean(-Q0(1,(Ncyc-1)*Nstep + 1:end))/Cpow_avg
