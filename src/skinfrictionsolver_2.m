...
function [dFi,dL_pct,factor,xbc_sep,xtc_sep,Indsep_b,Indsep_t,Stagpt] = SkinFrictionSolver(xc,Qt,nu,rho,c,tmax,Cp,Qinf)
chord = c;
% Re = 6e6;
% trans = 5e5;
% alpha = 15*(pi/180);
...
% plot(s_t,Cp_t,'-b')
% 
% xlabel('$$s$$','interpreter','latex','fontname','TimesNewRoman','fontsize',12)
...
% hold on
% 
% plot(s_b,1-Cpbar_b,'-k')
% plot(s_t,1-Cpbar_t,'-b')
% 
% xlabel('$$s$$','interpreter','latex','fontname','TimesNewRoman','fontsize',12)
% ylabel('$$1-\bar{C_p}$$','interpreter','latex','fontname','TimesNewRoman','fontsize',12)
% legend('Bottom','Top','Location','South')
...
% 
% advgrad_b = find(dCpdx_b(2:end-factor*3) > 0,1,'first');
% advgrad_t = find(dCpdx_t(2:end-factor*3) > 0,1,'first');
% 
% if isempty(advgrad_b)
%     advgrad_b = length(s_b);
% end
% if isempty(advgrad_t)
%     advgrad_t = length(s_t);
% end
% 
% s_b(advgrad_b)
% 
...
% % Calculating transformed Reynolds number.
% Re_xbar_b = ub_m*xbar_b/nu;
% Re_xbar_t = ut_m*xbar_t/nu;
% 
% F_b = Cpbar_b.*sqrt(xbar_b.*dCpbardx_b)./(10^(-6).*Re_xbar_b).^(0.1);
% F_t = Cpbar_t.*sqrt(xbar_t.*dCpbardx_t)./(10^(-6).*Re_xbar_t).^(0.1);
% 
% % F_b = Cpbar_b.*sqrt(xbar_b.*dCpdx_b)./(10^(-6).*Re_xbar_b).^(0.1);
% % F_t = Cpbar_t.*sqrt(xbar_t.*dCpdx_t)./(10^(-6).*Re_xbar_t).^(0.1);
% % 
% F_b = F_b.*(xbar_b > 0).*(4/7 >= Cpbar_b);
% F_t = F_t.*(xbar_t > 0).*(4/7 >= Cpbar_t);
% 
% F_b = real(F_b);
% F_t = real(F_t);
...
% plot(s_b,F_b,'-k')
% plot(s_t,F_t,'-b')
% plot(s_t,0.3*ones(length(s_t)),':r')
% 
% xlabel('$$s$$','interpreter','latex','fontname','TimesNewRoman','fontsize',12)
% ylabel('$$F(\bar{x})$$','interpreter','latex','fontname','TimesNewRoman','fontsize',12)
% legend('Bottom','Top','Location','South')


% if sum(F_b >= 0.5)
%     turbsep_b = find(F_b >= 0.5,1,'first');
% elseif sum(F_b >= 0.3)
%     [~,turbsep_b] = max(F_b);
% else
%     turbsep_b = [];
...
% 
% if sum(F_t >= 0.5)
%     turbsep_t = find(F_t >= 0.5,1,'first');
% elseif sum(F_t >= 0.3)
%     [~,turbsep_t] = max(F_t);
% else
%     turbsep_t = [];
...
% bot = sqrt(Cp_b).*s_b.*dCpbardx_b
% top = sqrt(Cp_t).*s_t.*dCpbardx_t
