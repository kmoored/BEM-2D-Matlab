...
for i = 1:Nelements
    if nodes(i,1) <= 0.5*tmax
        ...
        ttemp(i,1) = -tmax / (max(xp_0) - 0.5*tmax) * nodes(i,1) + tmax / (max(xp_0) - 0.5*tmax) * max(xp_0);
        tBeam(i-1,1) = 0.5*(ttemp(i,1) + ttemp(i-1,1));
        tBeamStruct(i-1,1) = tBeam(i-1,1);
        if i == Nelements
            tBeam(i,1) = 0.5*ttemp(i,1);
            tBeamStruct(i,1) = tBeam(i,1);
