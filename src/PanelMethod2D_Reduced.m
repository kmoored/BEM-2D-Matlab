...
% function [Ct_avg,Cl_avg,Cpow_avg,np,Ct_s,Cl_s,Cpow_s,Ct_us,Cl_us,Cpow_us] = PanelMethod2D_Comparison(d_c,St,Npanels,Nstep,Ncyc,TEfac,val)
% Npanels
% Nstep
% Ncyc
% St
...
%         zstar_b = zstar_b.*fence + (zstar_b - Q0(2,i_t-1)*delT).*(~fence);
    ...
    [xp,zp,Vp] = Kinematics_Pitch2D(xp(:,i_t),zp(:,i_t),alpha_max_ru,f,t,phi,tmax); 
    xpbod = xp;
    zpbod = zp;
...
%     zTE(:,i_t) = [zp(end,i_t); zp(end,i_t) + TEvec(2)*c*TEfac];
    ...
    % Moving first and last collocation points closer to the TE.
%     
%     Xc(Npanels) = (xc(Npanels,i_t) - 3/4*abs(xc(Npanels,i_t) - xp(Npanels,i_t))) - abs(val*zcval).*vn(Npanels,1,i_t);
%     Zc(Npanels) = (zc(Npanels,i_t) + 3/4*abs(zc(Npanels,i_t) - zp(Npanels,i_t))) - abs(val*zcval).*vn(Npanels,2,i_t);
%     
%     Xc(1) = (xc(1,i_t) - 3/4*abs(xc(1,i_t) - xp(1,i_t))) - abs(val*zcval).*vn(1,1,i_t);
%     Zc(1) = (zc(1,i_t) - 3/4*abs(zc(1,i_t) - zp(1,i_t))) - abs(val*zcval).*vn(1,2,i_t);
    
%     Xc(Npanels) = Xc(Npanels) - 3/4*abs(xc(Npanels,i_t) - xp(Npanels+1,i_t));
%     Zc(Npanels) = zc(Npanels,i_t);

%     Xc(1) = Xc(1) - 3/4*abs(xc(1,i_t) - xp(1,i_t));
%     Zc(1) = zc(1,i_t);
...
%     % Applying explicit Kutta condition.
%     A(Npanels+1,[1:Npanels+1]) = zeros(1,Npanels+1);
%     A(Npanels+1,[1 Npanels+1]) = 1;
%     A(Npanels+1,Npanels) = -1;
%     
%     % Defining Right Hand Side (RHS)
%     if LES == 1 && i_t > 2 
%         RHS = [-B*sigma(:,i_t) - wakeinf*Cw*muW' - wakeinf*CLEwt*muLEt' - wakeinf*CLEwb*muLEb';0];
%     elseif i_t > 1
%         RHS = [-B*sigma(:,i_t) - wakeinf*Cw*muW';0];
%     else
%         RHS = [-B*sigma(:,i_t);0];
%     end
    ...
    Qp(1,i_t) = SecondOrderForwDiff(mu(:,i_t),dL(:,i_t),[1, 2, 3]);
    Qp(2,i_t) = SecondOrderCenForwDiff(mu(:,i_t),dL(:,i_t),[1, 2, 3, 4]);
...
%     Qp(3:end-2,i_t) = (mu(2:end-3,i_t)- mu(4:end-1,i_t))./(1/2*dL(2:end-3,i_t) + dL(3:end-2,i_t) + 1/2*dL(4:end-1,i_t));
    ...
    for j = 1:Npanels-4  
        Qp(j+2,i_t) = CentDiff(mu(:,i_t),dL(:,i_t),Pan(:,j),Order);
    end

    Qp(Npanels-1,i_t) = SecondOrderCenBackDiff(mu(:,i_t),dL(:,i_t),[(Npanels - 3), (Npanels - 2), (Npanels - 1), Npanels]);
    Qp(Npanels,i_t) = SecondOrderBackDiff(mu(:,i_t),dL(:,i_t),[(Npanels - 2), (Npanels - 1), Npanels]);
...
%     Qt(1,i_t) = Qt(1,i_t) - 1/2*sum([Uinf Winf].*vt(1,:,i_t),2);
%     Qt(Npanels,i_t) = Qt(Npanels,i_t) - 1/2*sum([Uinf Winf].*vt(Npanels,:,i_t),2);
...
%     end
    ...
    if ViscDrag
        ...
        dFshear = kron(eye(Npanels),ones(1,factor))*dFi;
    else
        dFshear = zeros(Npanels,1);
    end
...
%% Calculating the forces
    ...
    % edge.
    delFp = -kron((Cp(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)*b),[1 1]).*vn(:,:,i_t);
    delFs = ViscDrag*kron(dFshear,[1 1]).*vt(:,:,i_t);
    delF = delFp + delFs;
    delP = sum(-delF.*Vc(:,:,i_t),2);
    ...
    delFp_s = -kron((Cp_s(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)*b),[1 1]).*vn(:,:,i_t);
    delFp_us = -kron((Cp_us(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)*b),[1 1]).*vn(:,:,i_t);
    delF_s = delFp_s;
    delF_us = delFp_us;
    delP_s = sum(-delF_s.*Vc(:,:,i_t),2);
    delP_us = sum(-delF_us.*Vc(:,:,i_t),2);
...
%     print('-dpng','-r300',['Free_png/2D_Pitch_Free_',num2str(i_t)]);
%     pause(0.1);
...
Cl_last = Cl((Ncyc-2)*Nstep+1:end);
hold on   
    plot([0 2],[0 0],'-k','linewidth',1)
    plot([0 2],mean(Cl_last)*[1 1],'--r','linewidth',3)
    plot(delt,Cl_last,'-b','linewidth',3)
hold off

axis([0 2 min(Cl_last) max(Cl_last)])
set(gca, 'FontName', 'TimesNewRoman', 'FontSize', FontSizeAx)
set(gca, 'Units', 'normalized', 'Position', axespos);
xlabel('$$t/T$$','FontName', 'TimesNewRoman','FontUnit', 'points','FontSize', FontSizeLb,'FontWeight', 'normal','Rotation', 0,'Units', 'Normalize','Position', xlabelpos,'Interpreter', 'LaTeX');
