...
delF_s = zeros(Npanels,2,Ncyc*Nstep+1);
delFp_s = zeros(Npanels,2,Ncyc*Nstep+1);
delP_s = zeros(Npanels,Ncyc*Nstep+1);
...
delF_us = zeros(Npanels,2,Ncyc*Nstep+1);
delFp_us = zeros(Npanels,2,Ncyc*Nstep+1);
delP_us = zeros(Npanels,Ncyc*Nstep+1);
...
%         zstar_b = zstar_b.*fence + (zstar_b - Q0(2,i_t-1)*delT).*(~fence);
    ...
    [xptemp,zptemp,Vptemp] = Kinematics_Pitch2D(xp(:,i_t),zp(:,i_t),alpha_max_ru,f,t,phi,tmax); 
    Vp(:,:,i_t) = Vptemp';
    xpbod(:,i_t) = xptemp';
    zpbod(:,i_t) = zptemp';
    xp(:,i_t) = xptemp';
...
%     end
    ...
    delFp_s(:,:,i_t) = -kron((Cp_s(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)*b),[1 1]).*vn(:,:,i_t);
    delFp_us(:,:,i_t) = -kron((Cp_us(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)*b),[1 1]).*vn(:,:,i_t);
    delF_s(:,:,i_t) = delFp_s(:,:,i_t);
    delF_us(:,:,i_t) = delFp_us(:,:,i_t);
    delP_s(:,i_t) = sum(-delF_s(:,:,i_t).*Vc(:,:,i_t),2);
    delP_us(:,i_t) = sum(-delF_us(:,:,i_t).*Vc(:,:,i_t),2);
    
    F_s(1,:,i_t) = sum(delF_s(:,:,i_t),1);
    Pow_s(1,i_t) = sum(delP_s(:,i_t),1);
    Fx_s(1,i_t) = F_s(1,1,i_t);
    Fz_s(1,i_t) = F_s(1,2,i_t);
    
    F_us(1,:,i_t) = sum(delF_us(:,:,i_t),1);
    Pow_us(1,i_t) = sum(delP_us(:,i_t),1);
