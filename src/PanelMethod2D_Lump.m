...
% function [Ct_avg,Cl_avg,Cpow_avg,np,Ct_s,Cl_s,Cpow_s,Ct_us,Cl_us,Cpow_us] = PanelMethod2D_Reduced_v2(d_c,St,Npanels,Nstep,Ncyc,TEfac,val)
% Npanels
% Nstep
% Ncyc
% St
...
phi = -(8/10)*pi/2;                   % Phase offset of actuation signal.
M = 2;
...
savefilename = ['Data_free_lump_4_Pitch_St',num2str(St*100),...
...
dLc = zeros(Npanels-1,Ncyc*Nstep+1);

xTE = zeros(2,Ncyc*Nstep+1);
zTE = zeros(2,Ncyc*Nstep+1);
dLTE = zeros(1,Ncyc*Nstep+1);
xw = zeros(1,Ncyc*Nstep+1);
zw = zeros(1,Ncyc*Nstep+1);
xl = zeros(1,Ncyc*Nstep);
zl = zeros(1,Ncyc*Nstep);
...
delF = zeros(Npanels,2);
delFp = zeros(Npanels,2);
delFs = zeros(Npanels,2);
delP = zeros(Npanels,1);
delF_s = zeros(Npanels,2);
delFp_s = zeros(Npanels,2);
delP_s = zeros(Npanels,1);
delF_us = zeros(Npanels,2);
delFp_us = zeros(Npanels,2);
delP_us = zeros(Npanels);

F_s = zeros(1,2,Ncyc*Nstep+1);
Fx_s = zeros(1,Ncyc*Nstep+1);
Fz_s = zeros(1,Ncyc*Nstep+1);
L_s = zeros(1,Ncyc*Nstep+1);
T_s = zeros(1,Ncyc*Nstep+1);
Pow_s = zeros(1,Ncyc*Nstep+1);

F_us = zeros(1,2,Ncyc*Nstep+1);
Fx_us = zeros(1,Ncyc*Nstep+1);
Fz_us = zeros(1,Ncyc*Nstep+1);
L_us = zeros(1,Ncyc*Nstep+1);
T_us = zeros(1,Ncyc*Nstep+1);
Pow_us = zeros(1,Ncyc*Nstep+1);
...
Cl_s = zeros(1,Ncyc*Nstep+1);
Ct_s = zeros(1,Ncyc*Nstep+1);
Cpow_s = zeros(1,Ncyc*Nstep+1);
Cl_us = zeros(1,Ncyc*Nstep+1);
Ct_us = zeros(1,Ncyc*Nstep+1);
Cpow_us = zeros(1,Ncyc*Nstep+1);
...
if grd == 1
    ...
    xl_2 = zeros(1,Ncyc*Nstep+1);
    zl_2 = zeros(1,Ncyc*Nstep+1);
    ...
    vtlw_2 = zeros(Ncyc*Nstep,2);
    vnlw_2 = zeros(Ncyc*Nstep,2);
else
    xTE_2 = [];
    zTE_2 = [];
    xw_2 = [];
...
for i_t = 1:Ncyc*Nstep+1
    ...
    delzp = (z_b(1,i_t))*ones(Npanels+1,1);
    delxc = x_b(1,i_t)*ones(Npanels,1);
    delzc = (z_b(1,i_t))*ones(Npanels,1);
...
%         
    ...
    xTE(:,i_t) = [xp(end); xp(end) + TEvec(1)*Qinf*delT*TEfac];
    zTE(:,i_t) = [zp(end); zp(end) + TEvec(2)*Qinf*delT*TEfac];

    dLTE(i_t) = sqrt((xTE(2,i_t)-xTE(1,i_t))^2 + (zTE(2,i_t)-zTE(1,i_t))^2);
    ...
    if grd == 1
        ...
        vtTE_2 = [vtTE(1) -vtTE(2)];
        vnTE_2 = [vnTE(1) -vnTE(2)];
    end
    ...
    if t > 0
        ...
        muLump(2:end) = muLump(1:end-1);
        xl(2:end) = xl(1:end-1);
        zl(2:end) = zl(1:end-1);
        
        vtlw(2:end,:) = vtlw(1:end-1,:);
        vnlw(2:end,:) = vnlw(1:end-1,:);
...
%             zw_2(1,1:2) = -zw(1,1:2);
            ...
            vtlw_2 = [vtlw(:,1) -vtlw(:,2)];
            vnlw_2 = [vnlw(:,1) -vnlw(:,2)];
    ...
    B = zeros(Npanels,Npanels);
    Cb = zeros(Npanels,Npanels);
    dPhi_s_2 = zeros(Npanels,Npanels);
    dPhi_d_2 = zeros(Npanels,Npanels);
    
    [dPhi_s,dPhi_d,dLtemp] = Phi(ones(1,Npanels),ones(1,Npanels),Xc',Zc',xp',zp',vt',vn');
    dL(:,i_t) = dLtemp;
    
    if grd == 1
        [dPhi_s_2,dPhi_d_2,dLtemp2] = Phi(ones(1,Npanels),ones(1,Npanels),Xc',Zc',xp_2',zp_2',vt_2',vn_2');
    end
...
%%%%%%   
    % Influence of the wake panels    
     if i_t > 1   
        ...
        if grd == 1
            [~,dPhi_d_2,~] = Phi(ones(1,length(muW(1:wakeInd))),zeros(1,length(muW(1:wakeInd))),Xc',Zc',xw_2(1:wakeInd+1),zw_2(1:wakeInd+1),vtw_2(1:wakeInd,:)',vnw_2(1:wakeInd,:)');
        end
...
%%%%%%   
    % Influence of the lumped wake panel    
     if i_t > 1   
        [~,dPhi_d,~] = Phi(1,0,Xc',Zc',[xw(Nlump*Nstep+1) xl(1)],[zw(Nlump*Nstep+1) zl(1)],vtlw(1,:)',vnlw(1,:)');
        
        dPhi_d_2 = 0*dPhi_d;
        
        if grd == 1
            [~,dPhi_d_2,~] = Phi(1,0,Xc',Zc',[xw_2(Nlump*Nstep+1) xl_2(1)],[zw_2(Nlump*Nstep+1) zl_2(1)],vtlw_2(1,:)',vnlw_2(1,:)');
        end
...
%%%%%%
    ...
    if LES == 1 && i_t > 2 
        RHS = -B*sigma(:,i_t) - wakeinf*Cw*muW' - wakeinf*Clw*muLump(1) - wakeinf*CLEwt*muLEt' - wakeinf*CLEwb*muLEb';
    elseif i_t > 1
        RHS = -B*sigma(:,i_t) - wakeinf*Cw*muW' - wakeinf*Clw*muLump(1);
    else
        RHS = -B*sigma(:,i_t);
    end
    ...
    if i_t == 1
        GammaW(1,1) = -muTE(1,i_t);
    else
        GammaW(1,1) = -(muTE(1,i_t) - muTE(1,i_t - 1));
    end
    ...
    Qp(1) = SecondOrderForwDiff(mu(:,i_t),dL(:,i_t),[1, 2, 3]);
    Qp(2) = SecondOrderCenForwDiff(mu(:,i_t),dL(:,i_t),[1, 2, 3, 4]);
...
%     Qp(3:end-2,i_t) = (mu(2:end-3,i_t)- mu(4:end-1,i_t))./(1/2*dL(2:end-3,i_t) + dL(3:end-2,i_t) + 1/2*dL(4:end-1,i_t));
    ...
    for j = 1:Npanels-4  
        Qp(j+2) = CentDiff(mu(:,i_t),dL(:,i_t),Pan(:,j),Order);
    end

    Qp(Npanels-1) = SecondOrderCenBackDiff(mu(:,i_t),dL(:,i_t),[(Npanels - 3), (Npanels - 2), (Npanels - 1), Npanels]);
    Qp(Npanels) = SecondOrderBackDiff(mu(:,i_t),dL(:,i_t),[(Npanels - 2), (Npanels - 1), Npanels]);
    ...
    if ViscDrag
        [dFi_pct,dL_pct,factor,xbc_sep,xtc_sep,Indsep_b,Indsep_t,Stagpt] = SkinFrictionSolver(xc,Qt,nu,rho,c,tmax,Cp(:,i_t),Qinf);   
...
%% Calculating the forces
    ...
    % edge.
    delFp = -kron((Cp(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)*b),[1 1]).*vn;
    delFs = ViscDrag*kron(dFshear,[1 1]).*vt;
    delF = delFp + delFs;
    delP = sum(-delF.*Vc,2);  
    
    F(1,:,i_t) = sum(delF,1);
    Pow(1,i_t) = sum(delP,1);
    Fx(1,i_t) = F(1,1,i_t);
    Fz(1,i_t) = F(1,2,i_t);
    ...
    Gamma(1,i_t) = mu(Npanels,i_t) - mu(1,i_t);
    ...
    delFp_s = -kron((Cp_s(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)*b),[1 1]).*vn;
    delFp_us = -kron((Cp_us(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)*b),[1 1]).*vn;
    delF_s = delFp_s;
    delF_us = delFp_us;
    delP_s = sum(-delF_s.*Vc,2);
    delP_us = sum(-delF_us.*Vc,2);
    
    F_s(1,:,i_t) = sum(delF_s,1);
    Pow_s(1,i_t) = sum(delP_s,1);
    Fx_s(1,i_t) = F_s(1,1,i_t);
    Fz_s(1,i_t) = F_s(1,2,i_t);
    
    F_us(1,:,i_t) = sum(delF_us,1);
    Pow_us(1,i_t) = sum(delP_us,1);
    Fx_us(1,i_t) = F_us(1,1,i_t);
    Fz_us(1,i_t) = F_us(1,2,i_t);
    
    L_s(1,i_t) = Fz_s(1,i_t)*cos(alpha) - Fx_s(1,i_t)*sin(alpha);
    T_s(1,i_t) = -(Fz_s(1,i_t)*sin(alpha) + Fx_s(1,i_t)*cos(alpha));
    
    Cl_s(1,i_t) = L_s(1,i_t)/(1/2*rho*Qinf^2*c*b);
    Ct_s(1,i_t) = T_s(1,i_t)/(1/2*rho*Qinf^2*c*b);
    Cpow_s(1,i_t) = Pow_s(1,i_t)/(1/2*rho*Qinf^3*c*b);
    
    L_us(1,i_t) = Fz_us(1,i_t)*cos(alpha) - Fx_us(1,i_t)*sin(alpha);
    T_us(1,i_t) = -(Fz_us(1,i_t)*sin(alpha) + Fx_us(1,i_t)*cos(alpha));
    
    Cl_us(1,i_t) = L_us(1,i_t)/(1/2*rho*Qinf^2*c*b);
    Ct_us(1,i_t) = T_us(1,i_t)/(1/2*rho*Qinf^2*c*b);
    Cpow_us(1,i_t) = Pow_us(1,i_t)/(1/2*rho*Qinf^3*c*b);
    ...
    if Rollup == 1 && i_t > 1
        ...
        [u,w] = WakeRollupVelocity2D([xw(2:wakeInd+1) lump*xl(1)],[zw(2:wakeInd+1) lump*zl(1)],mu(:,i_t)',muTE(i_t),[muW(1:wakeInd) lump*muLump(1)],muLEt,muLEb,sigma(:,i_t)',xp',zp',vt',vn',xTE(:,i_t)',zTE(:,i_t)',vtTE(1,:,i_t)',vnTE(1,:,i_t)',[xw(1:wakeInd+1) lump*xl(1)],[zw(1:wakeInd+1) lump*zl(1)],[vtw(1:wakeInd,:); lump*vtlw(1,1) lump*vtlw(1,2)]',[vnw(1:wakeInd,:); lump*vnlw(1,1) lump*vnlw(1,2)]',xpt_LES,zpt_LES,vtLEt,vnLEt,xpb_LES,zpb_LES,vtLEb,vnLEb,i_t,Ncyc,Nstep,LES,ep,epBod,SC);
                
        u_2 = 0*u;
        w_2 = 0*w;
        
        if grd == 1
            [u_2,w_2] = WakeRollupVelocity2D([xw(2:wakeInd+1) lump*xl(1)],[zw(2:wakeInd+1) lump*zl(1)],mu(:,i_t)',muTE(i_t),[muW(1:wakeInd) lump*muLump(1)],muLEt,muLEb,sigma(:,i_t)',xp_2',zp_2',vt_2',vn_2',xTE_2(:,i_t)',zTE_2(:,i_t)',vtTE_2(1,:,i_t)',vnTE_2(1,:,i_t)',[xw_2(1:wakeInd+1) lump*xl_2(1)],[zw_2(1:wakeInd+1) lump*zl_2(1)],[vtw_2(1:wakeInd,:); lump*vtlw_2(1,1) lump*vtlw_2(1,2)]',[vnw_2(1:wakeInd,:); lump*vnlw_2(1,1) lump*vnlw_2(1,2)]',xpt_LES,zpt_LES,vtLEt,vnLEt,xpb_LES,zpb_LES,vtLEb,vnLEb,i_t,Ncyc,Nstep,LES,ep,epBod,SC);
        end
        
        % Applying fencing scheme
        [xstar_w,zstar_w,fence] = fencing(c,Vp,[xw(2:wakeInd+1) lump*xl(1)],[zw(2:wakeInd+1) lump*zl(1)],u + u_2,w + w_2,xp,zp,vn,x_b(1,i_t),z_b(1,i_t),delT,1/100*epBod);
        ...
        if grd == 1
            % Wake rollup
            xw_2(2:wakeInd+1) = xw(2:wakeInd+1);
            zw_2(2:wakeInd+1) = -zw(2:wakeInd+1);
            ...
            vtlw_2(1,:) = [vtlw(1,1)  -vtlw(1,2)];
            vnlw_2(1,:) = [vnlw(1,1)  -vnlw(1,2)];
        end
...
%     vec = vec(end:-1:1,:);
    ...
    WakeCirc = GammaW'/max(GammaW)/(1/1);
    ...
    WakeCirc_w = -GammaW'/max(GammaW)/(1/1);
    ...
    WakeCirc_l = -muLump(1)'/max(GammaW)/(1/1);
    ...
    if grd == 1 && i_t > 1
        for i_w = 1:wakeInd+1
            plot(xw_2(i_w),zw_2(i_w),'.','color',vec(WakeCirc_w(i_w),:),'linewidth',2,'markersize',14)
        end
    end
    if i_t > Nlump*Nstep + 1
...
%     axis([-c/2 + x_b(i_t) 2*c + x_b(i_t) -1/2*c + z_b(i_t) 1/2*c + z_b(i_t)])
    axis([-c/2 + x_b(i_t) 6*c + x_b(i_t) z_b(i_t) - 1.5*c z_b(i_t) + 1.5*c])
...
%         zf = linspace(-3*c/4 + z_b(i_t),3*c/4 + z_b(i_t),nz)';
        ...
        % Body contribution
        [u_st,w_st,u_bdt,w_bdt] = DubSorV(mu(:,i_t)',sigma(:,i_t)',X,Z,xp',zp',vt',vn',epSC,SC);
        ...
        if grd == 1
            [u_st_2,w_st_2,u_bdt_2,w_bdt_2] = DubSorV(mu(:,i_t)',sigma(:,i_t)',X,Z,xp_2',zp_2',vt_2',vn_2',epSC,SC);
        end
...
%         colorbar
        caxis(6*[-1 1])
        colorbar
        axis([-c/4 + x_b(i_t) 2*c + x_b(i_t) -c c])
...
%         print('-dpng','-r300',['2D_NearBody_',num2str(i_t)]);
    ...
    if free == 1 && i_t > 1
        ...
        hold on
        plot(((1:i_t) - 1)*delT*f,-Q0(1,1:i_t)'/(rho*c*b*f*(A_c*c)^2/M),'-b','linewidth',2)
        hold off
        ...
        ylabel('$$\frac{U m}{ \rho c b f A^2}$$','Interpreter', 'LaTeX','FontName', 'TimesNewRoman','FontUnit', 'points','FontSize', FontSizeLb,'FontWeight', 'normal','Rotation', 90,'Units', 'Normalize','Position',ylabelpos);
        axis([0 Ncyc/f 0 1.5*max(-Q0(1,1:i_t)/(rho*c*b*f*(A_c*c)^2/M))])
...
T_avg = real(mean(T(1,(Ncyc-1)*Nstep+1:end)))
Pow_avg = real(mean(Pow(1,(Ncyc-1)*Nstep+1:end)))
Cl_avg = mean(Cl(1,(Ncyc-1)*Nstep+1:end))
Ct_avg = real(mean(Ct(1,(Ncyc-1)*Nstep+1:end)))
Cpow_avg = real(mean(Cpow(1,(Ncyc-1)*Nstep+1:end)))
...
Cl_last = Cl((Ncyc-3)*Nstep+1:end);
hold on   
    plot([0 Ncyc],[0 0],'-k','linewidth',1)
%     plot([0 3],Cl_avg*[1 1],'--r','linewidth',3)
    plot(delt,Cl,'-b','linewidth',3)
hold off

axis([0 Ncyc min(Cl) max(Cl)])
set(gca, 'FontName', 'TimesNewRoman', 'FontSize', FontSizeAx)
set(gca, 'Units', 'normalized', 'Position', axespos);
xlabel('$$t/T$$','FontName', 'TimesNewRoman','FontUnit', 'points','FontSize', FontSizeLb,'FontWeight', 'normal','Rotation', 0,'Units', 'Normalize','Position', xlabelpos,'Interpreter', 'LaTeX');
