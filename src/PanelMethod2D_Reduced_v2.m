...
for i_t = 1:Ncyc*Nstep+1
    ...
    if i_t > Nlump*Nstep+1
        muTot = muLump - GammaW(Nlump*Nstep+1);

        xl = abs(muLump/muTot)*xl + abs(-GammaW(Nlump*Nstep+1)/muTot)*xw(Nlump*Nstep+1);
        zl = abs(muLump/muTot)*zl + abs(-GammaW(Nlump*Nstep+1)/muTot)*zw(Nlump*Nstep+1);
...
%     xTE(:,i_t) = [xp(end,i_t); xp(end,i_t) + TEvec(1)*TEfac*c];
%     zTE(:,i_t) = [zp(end,i_t); zp(end,i_t) + TEvec(2)*TEfac*c];
...
%         % Upstream fencing
%         upfence = 3/4; 
%         xw((xw(Ncyc*Nstep + 3 - i_t:end) - xp(Npanels/2 + 1)) <= upfence*c) = xp(Npanels/2 + 1) + upfence*c;
%        
        vttemp = [(xw(1,Ncyc*Nstep + 3 - i_t:end) - xw(1,Ncyc*Nstep + 2 - i_t:end-1))' (zw(1,Ncyc*Nstep + 3 - i_t:end) - zw(1,Ncyc*Nstep + 2 - i_t:end-1))'];
        vtw(Ncyc*Nstep + 2 - i_t:end,:) = diag(1./sqrt(vttemp(:,1).^2 + vttemp(:,2).^2))*vttemp;
        vnw(Ncyc*Nstep + 2 - i_t:end,:) = [-vtw(Ncyc*Nstep + 2 - i_t:end,2) vtw(Ncyc*Nstep + 2 - i_t:end,1)]; 
...
%     vec = vec(end:-1:1,:);
    ...
    for i_w = Ncyc*Nstep + 2 - i_t:Ncyc*Nstep+1
        ...
        plot(xl,zl,'.','color','k','linewidth',2,'markersize',14)
    end
    if grd == 1 && i_t > 1
        for i_w = Ncyc*Nstep + 2 - i_t:Ncyc*Nstep+1
            plot(xw_2(i_w),zw_2(i_w),'.','color',vec(WakeCirc_w(i_w),:),'linewidth',2,'markersize',14)
        end
        % Plot doublet panels as well as end-points
%         for i_w = Ncyc*Nstep + 2 - i_t:Ncyc*Nstep
%             plot([xw_2(i_w) xw_2(i_w+1)],[zw_2(i_w) zw_2(i_w+1)],'.-','color',vec(WakeCirc_w(i_w),:),'linewidth',2,'markersize',14)
%         end
    end
...
%     print('-dpng','-r300',['Grd_png/2D_Pitch_Grd_',num2str(i_t)]);
%     pause(0.1);
...
if SaveData == 1
    save(['Data_GrdEffect2D_2/Pitch_St',num2str(St*100),...
        '_A_c',num2str(A_c*100),...
        '_d_c',num2str(d_c*100),'.mat'],'-v7.3');
end
