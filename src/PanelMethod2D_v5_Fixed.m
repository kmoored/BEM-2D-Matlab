...
% function [Ct_avg,Tnet_avg,D_avg,Cl_avg,Cpow_avg,Pow_avg,np,Ec,f,A_TE,rho,c,Qinf] = PanelMethod2D_v5_Fixed(d_c,f,A_c,Re,Npanels,Nstep,Ncyc,Nlump,TEfac,val)
% Npanels
% Nstep
% Ncyc
% Nlump
...
phi = -pi/4; %pi-(8/10)*pi/2;      % Phase offset of actuation signal.
% M = 1000;

% Virtual body properties
% M_star = 2;
% Beta = 1;
% M = M_star*rho*Beta*(c*b)*(A_c*c)
% A_bp = 50;
% A_bod = A_bp*(c*b);
% Cd_bod = 0.015;
% Ct_prop = 0.55;

% Free-swimming speed and number of cycle to reach steady-state.
% U_swim = 2*f*(A_c*c)*sqrt(2*Beta/A_bp*(Ct_prop/Cd_bod))
% N_a = M_star/Ct_prop*sqrt(2*Beta*(1/A_bp)*(Ct_prop/Cd_bod))
...
%             '_d_c_inf'];
    ...
    save(['FixedV/Valid_Parameters',savefilename,'.mat'],'-v7.3')

    fid_Data = fopen(['FixedV/Valid_Data',savefilename,'.txt'],'w');
    fid_PanelProp = fopen(['FixedV/Valid_PanelProp',savefilename,'.txt'],'w');
    fid_WakeProp = fopen(['FixedV/Valid_WakeProp',savefilename,'.txt'],'w');
end
...
%         print('-dpng','-r300',['2D_NearBody_',num2str(i_t)]);
    ...
    if free == 1 && i_t > 1
        ...
        hold on
            plot([0 Ncyc],U_swim*[1 1],'-k','linewidth',1)
            plot(N_a*[1 1],[0 1.5*max(-Q0(1,1:i_t))],'--k','linewidth',1)
            plot(((1:i_t) - 1)*delT*f,-Q0(1,1:i_t)','-b','linewidth',2)
        hold off
...
np = Ct_avg/Cpow_avg
Ec = mean(-Q0(1,(Ncyc-1)*Nstep + 1:end))/Pow_avg   %km/kJ
