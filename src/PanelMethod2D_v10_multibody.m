...
% function [Ct_avg,Tnet_avg,D_avg,Cl_avg,Cpow_avg,Pow_avg,np,Ec,f,A_TE,rho,c,Ucruise,V_t] = PanelMethod2D_v8(A_bp,M_star,St,A_c,c,Qinf,d_c,Npanels,Nstep,Ncyc,Nlump,TEfac,val)
...
% d_c = 1/2;                  % Distance from the ground, chords.
...
phi = 0;                    % Phase offset of actuation signal.
% M = 1000;
...
Cd_bod = 0.015;                 % Virtual body drag coefficient.
...
% f = St*Qinf/(2*h_c*c)                         % Frequency calculation, Hz.
...
if SaveData == 1
    ...
    savefilename = ['_Test_St',num2str(St),...
            '_h_c',num2str(h_c),...
            '_Qinf',num2str(Qinf)];
...
%     Qp(3:end-2,i_t) = (mu(2:end-3,i_t)- mu(4:end-1,i_t))./(1/2*dL(2:end-3,i_t) + dL(3:end-2,i_t) + 1/2*dL(4:end-1,i_t));
    ...
    Qt = Qp + sum(-Vt.*vt,2);
%     Qt = Qp + sum(-kron(ones(Npanels,1),Q0(:,i_t)').*vt,2);
%     Qp = repmat(Qp,1,2).*vt + repmat(sigma,1,2).*vn;
