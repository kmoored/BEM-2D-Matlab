...
function [Ct_avg,Cl_avg,Cpow_avg,np] = 
...
% Geometry parameters
c = 0.12;                      % m
tmax = 0.01;                % Percent of the chord
...
St = 0.1;
% f_red = 2.5;
alpha_max = 7.1808*(pi/180);     % Max pitch angle
...
%     Zc = zc(:,i_t) - abs(val*zcval).*vn(:,2,i_t);
    ...
    % Local velocity over the body due to the perturbation velocity.
...
%     end
    ...
    dFi = dFi_pct.*kron(dL(:,i_t),ones(factor,1));
...
%     end
    ...
    % edge.
    delF(:,:,i_t) = -kron((Cp(:,i_t)*1/2*rho*Qinf^2.*dL(:,i_t)),[1 1]).*vn(:,:,i_t) + ViscDrag*kron(dFshear(:,i_t),[1 1]).*vt(:,:,i_t);
    ...
    Cf(1,i_t) = norm(F(1,:,i_t))/(1/2*rho*Qinf^2*c);
    Cl(1,i_t) = L(1,i_t)/(1/2*rho*Qinf^2*c);
    Ct(1,i_t) = T(1,i_t)/(1/2*rho*Qinf^2*c);
    Cpow(1,i_t) = Pow(1,i_t)/(1/2*rho*Qinf^3*c);
...
%         plot(xp(Stagpt,i_t),zp(Stagpt,i_t),'xk','linewidth',2)
        plot(xw(Ncyc*Nstep + 2 - i_t:end),zw(Ncyc*Nstep + 2 - i_t:end),'.-b','linewidth',2,'markersize',14)
    end
...
% Plotting coefficient of lift and drag.
figure
set(gcf,'DefaultAxesfontsize',20,'DefaultAxesfontname','TimesNewRoman','DefaultAxesGridLineStyle','-.')

hold on
t = (Uinf/c*[1:Ncyc*Nstep + 1] - 1)*delT;
plot(t,Cl,'-b','linewidth',3)
plot(t,-Ct,'-r','linewidth',3)

axis([0 max(t) -1.1*max(Cl(3:end)) 1.1*max(Cl(3:end))])
% print('-dpng','-r300',['2D_Steady_forces']);
% xlabel('$$t, sec$$','interpreter','latex','fontsize',12,'fontname','TimesNewRoman')
% ylabel('$$C_l$$, $$C_t$$','interpreter','latex','fontsize',12,'fontname','TimesNewRoman')
