...
% function [Ct_avg,Tnet_avg,D_avg,Cl_avg,Cpow_avg,Pow_avg,np,Ec,f,A_TE,rho,c,Ucruise,V_t] = PanelMethod2D_v5(A_bp,M_star,f,A_c,c,Npanels,Nstep,Ncyc,Nlump,TEfac,val)
% Npanels
% Nstep
% Ncyc
% Nlump
...
% c = c_D*tmax;               % meters
b = 2*c;                    % meters
d_c = 0;                    % Distance from the ground in chords
...
phi = -pi/2; %pi-(8/10)*pi/2;      % Phase offset of actuation signal.
% M = 1000;
...
%             '_d_c_inf'];
    ...
    save(['FreeV_Visc/Parameters',savefilename,'.mat'],'-v7.3')

    fid_Data = fopen(['FreeV_Visc/Data',savefilename,'.txt'],'w');
    fid_PanelProp = fopen(['FreeV_Visc/PanelProp',savefilename,'.txt'],'w');
    fid_WakeProp = fopen(['FreeV_Visc/WakeProp',savefilename,'.txt'],'w');
end
