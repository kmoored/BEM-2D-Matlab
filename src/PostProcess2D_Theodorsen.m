...
savefilename = ['_Theodorsen_St',num2str(St),...
            '_h_c',num2str(h_c),...
            '_Qinf',num2str(Qinf)];

%% Loading data 
load(['Theodorsen/Parameters',savefilename,'.mat']);       
Data = load(['Theodorsen/Data',savefilename,'.txt']);
PanelProp = load(['Theodorsen/PanelProp',savefilename,'.txt']);
WakeProp = load(['Theodorsen/WakeProp',savefilename,'.txt']);
...
save(['Theodorsen/ProcessedData',savefilename,'.mat'],'-v7.3') 
...
%         pcolor(Xstar,Zstar,-omega_y)
        ...
        quiver(Xf,Zf,Ut(:,:,figInd),Wt(:,:,figInd),'b')
...
% Theordoson data St = 0.25, A_c = 0.05.

        
% load(['Theodorsen/ProcessedData',savefilename,'.mat']) 
...
hold on
%     plot(delt,Cl_s((Ncyc-1)*Nstep+1:end),'ob','linewidth',3)
%     plot(delt,Cl_us((Ncyc-1)*Nstep+1:end),'or','linewidth',3)
    plot(delt(1:5:end),Cl((Ncyc-1)*Nstep+1:5:end),'ok','linewidth',3)
    
%     plot(Theo_t,Cl_c_Theo,'-b','linewidth',3)
%     plot(Theo_t,Cl_a_Theo,'-r','linewidth',3)
    plot(Theo_t,Cl_Theo,'-k','linewidth',3)

%     plot([min(delt) max(delt)],mean(Cl((Ncyc-2)*Nstep+1:end))*[1 1],':k','linewidth',2)
    plot([min(delt) max(delt)],[0 0],'-k','linewidth',1)
hold off
...
print('-depsc','-r600',['Theodorsen/Validation',savefilename,'.eps']);
save(['Theodorsen/Validation',savefilename,'.mat'],'-v7.3','Cl','delt','Cl_Theo','Theo_t') 
